  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.from('.about-line', {
        scrollTrigger: { trigger: '.about-line', start: 'top 88%' },
        scaleX: 0, duration: 0.6, ease: 'power3.out', transformOrigin: 'left',
      });
      gsap.from('.about-bio', {
      ...
      gsap.from('.about-bullet', {
        scrollTrigger: { trigger: '.about-bullets', start: 'top 82%' },
    ...
    >
      ...
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, lineHeight: 0 }}>
        <svg viewBox="0 0 1440 60" preserveAspectRatio="none" style={{ width: '100%', height: '60px', display: 'block' }}>
          <path d="M0,40 C360,80 1080,0 1440,40 L1440,0 L0,0 Z" fill="#0a0a0f" />
        </svg>
      </div>
            ...
            <ul className="about-bullets" style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: '0.9rem' }}>
              {bullets.map((b, i) => (
                ...
                >
                  ...
                  <span style={{ fontFamily: '"Inter", sans-serif', fontSize: '0.9rem', color: '#8888a8', lineHeight: 1.6 }}>
                    {b.text}
                  </span>
                </li>
              ))}
          ...
          >
            ...
            >
              ...
              {[
                ...
                { bottom: -1, left: -1, borderBottom: '2px solid #00F5D4', borderLeft: '2px solid #00F5D4' },
                { bottom: -1, right: -1, borderBottom: '2px solid #00F5D4', borderRight: '2px solid #00F5D4' },
              ].map((style, i) => (
                <div key={i} style={{ position: 'absolute', width: '20px', height: '20px', borderRadius: '2px', ...style }} />
              ))}

              {/* Avatar placeholder */}
              <div style={{
                width: '100px', height: '100px', borderRadius: '50%',
                background: 'linear-gradient(135deg, #6C63FF 0%, #00F5D4 100%)',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                fontSize: '2.5rem', fontFamily: '"Oswald", sans-serif', fontWeight: 700,
                color: '#fff', boxShadow: '0 0 30px rgba(108,99,255,0.4)',
              }}>
                YN
              </div>

              <div style={{ textAlign: 'center' }}>
                <p style={{ fontFamily: '"Oswald", sans-serif', fontWeight: 600, fontSize: '1.15rem', color: '#e8e8e8' }}>
                  Your Name
                </p>
              ...
              <div style={{
                ...
                <span style={{ fontFamily: '"Fira Code", monospace', fontSize: '0.7rem', color: '#00F5D4' }}>
                  Open to work
                </span>
              </div>
            </div>
      ...
      <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, lineHeight: 0 }}>
        <svg viewBox="0 0 1440 60" preserveAspectRatio="none" style={{ width: '100%', height: '60px', display: 'block' }}>
          <path d="M0,20 C360,60 1080,-20 1440,20 L1440,60 L0,60 Z" fill="#0a0a0f" />
        </svg>
      </div>
    </section>
  );
